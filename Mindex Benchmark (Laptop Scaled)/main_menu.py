...
def main_menu(): #The Main Menu loop, operating under a define function
    while True:
        ...
        mouse_x, mouse_y = pygame.mouse.get_pos() #Setting the mouse positional variables for the main menu screen, uses different variable names to prevent variable clashing when executing the two gamemodes
        ...
        screen.blit(game1text, (button_1.centerx - game1text.get_width()/2, button_1.centery - game1text.get_height()/2)) #The centerx and centery string works to center the display blit to according to the dimensions of the button's drawn object
...
def game1(): #Calls the Target Clicking gamemode from the gamemodes folder
    target_clicking.run_game() 
 
def game2():
    decision_making.run_game2() #Calls the Decision Making gamemode from the gamemodes folder

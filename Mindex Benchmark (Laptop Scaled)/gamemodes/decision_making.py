...
def run_game2(): #This define function encapsulates the entire Decision Making gamemode, allowing it to be executed from the main menu file
    #Structure is copied from Target Clicking for base elements like points scoring, countdown, timers, and pre-game foundations
    pygame.init()
    pygame.font.init()
...
#Defining the screen size of the game
    ...
    def pre_timer(counter): #Sets the timer to display in a 00:00 format, by using an if logic to determine whether to add an extra 0 in front of the seconds variable
        if counter > 9:
            return '0:'
        else:
            return '0:0'
...
#Setting the countdown screen, porting the same code from Target Clicking
    def countdown(seconds):
        for i in range(seconds,0,-1):
            ...
            screen.blit(countdown_text,(w/2 - countdown_text.get_width()/2, h/2 - countdown_text.get_height()/2)) #Centering the countdown number on the screen basedon the width and height of the game window, and text element
            #This is due to how Pygame draws objects, starting from the top left corner of the object
            pygame.display.flip()
            time.sleep(1)

    countdown(3) #This runs the countdown functiond defined above, with a parameter of 3 seconds to allow players time to prepare
...
#Main Game Loop
    while running:
        ...
        accuracy_text = font.render(f"Accuracy: " + str(accuracy) + "%", True, (0,0,0))
                ...
                if mode == "center" and center_shape is not None:
                    if center_shape == "Rect": #A conditional that if the center shape variable is equivalent to a Rect 
                        if (mouse[0] < center_x + hitbox_x and mouse[0] > center_x) and (mouse[1] < center_y + hitbox_y and mouse[1] > center_y): #Matches Target Clicking hitscan logic exactly
                            click == True
                            correct_shape = "Rect"
                            mode = "randomized" #Switches the game mode to "randomized" after the correct center shape has been clicked, in which the code jumps to the "randomized" mode hitscan logic section below
                            hit_counter += 1
                            rect_reset = True 
...
#Setting the hitscanning logic when the game is in "randomized" mode, which receives the correct shape variable from the "center" mode logic above
                #Mirroring the hitscan mechanism when the game event mode is in "randomized". There are no scoring variables here because the center shape is meant to set the player with a specific target first.
                    ...
                    if clicked_shape == correct_shape: #This is the fundamental logic that checks if the clicked shape is the shape set to the correct state, thus points scoring will go under here
                        ...
                        hit_counter += 1
                        bonus_counter += 1
                        mode = "center" #Resets the game back to "center" mode after the correct shape in "randomized" mode has been clicked, bringing it back to "mode = center" portion of the logic above
                        center_shape = None
                    else:
                        score = score - 50
                        miss_counter += 1
...
#Drawing out the background color and the shapes
        screen.fill("Dark Gray")
        #When the game checks if the mode is "center", it draws the initial correct shape in the center of the screen based on the randomization of random.choice above
        if mode == "center" and center_shape is not None:
            if center_shape == "Rect": #Rect is assigned with green
                pygame.draw.rect(screen,"Green",(center_x, center_y, 100, 100)) 
                ...
                pygame.draw.ellipse(screen,"Red",(center_x , center_y, 100, 100)) 

        elif mode == "randomized": #Drawing the rectangle, triangle, and circle targets are drawn onto the canvas
            pygame.draw.rect(screen,"Green",(rect_x, rect_y,100,100)) 
            pygame.draw.ellipse(screen,"Red",(ellipse_x, ellipse_y,100,100))
        ...
        pygame.display.flip() #Updates the everything onto the canvas

#Points display
    print(f"Final Score:",score) #Prints the player's results onto the console 
    print(f"Accuracy:",accuracy,"%")

if __name__ == "__main__": #Allows Decision Making to run as a standalone program, taking guidance from Martin Breuss' article.
    run_game2()

...
def run_game(): #This define function encapsulates the entire Target Clicking gamemode, allowing it to be executed from the main menu file
    pygame.init()
    pygame.font.init()
...
#Setting the parameter variables for the targets
    ...
    def pre_timer(counter): #Sets the timer to display in a 00:00 format, by using an if logic to determine whether to add an extra 0 in front of the seconds variable
        #The red text block is an input string that adds a 0 in front of the seconds variable if it is less than 10, to maintain the 00:00 format
        if counter > 9: 
            return '0:'
        else:
            return '0:0'
...
#Setting the reset variables to be used later, alongside the running = True variable that is common in all PyGame game loops
    ...
    reset3 = True #Setting the base reset for the third target
    running = True #Common runnning variable for a game loop
...
#Creating the timer event 
    ...
    countdown(3) #Sets a countdown based on the set dimensions above starting from 3 seconds, and decreasing every second before going onto the main game, allowing players time to prepare
...
#Main Game Loop
    while running: 
        ...
        accuracy_text = font.render(f"Accuracy: " + str(accuracy) + "%", True, (0, 0, 0)) #Renders the accuracy variable as text on the screen with black text
...
#Randomizer element for each target orbs' x and y coordinates
        for event in pygame.event.get():
            if reset is True:
                ...
                reset = False #While the targets coordinates are not clicked, the reset variable remains false to prevent the target from constantly changing position
            if reset2 is True:
            ...
            if reset3 is True:
                ...
                reset3 = False
            if event.type == pygame.QUIT or counter == 0: #Ends the session if the timer reaches 1, as putting 0 will make the timer go into a negative number. However, because the tick setting is 1, it will end at 0 regardless.
                running = False
...
#Hitscan mechanism for all three targets           
            if event.type == pygame.MOUSEBUTTONDOWN: #Setting the primary control with an event where a mouse button is clicked
                if (mouse[0] < target_x + hitbox_x) and (mouse[0] > target_x) and (mouse[1] < target_y + hitbox_y) and (mouse[1] > target_y) and event.button == 1: #This checks if the mouse position is within the target's spawnpoint with target_x added with hitbox_x, and same for target_y and hitbox_y.
                    ...
                    score_text = font.render(f"Score: " + str(score), True, (0, 0, 0)) #This renders the score variable as text on the screen with black text. Takes guidance from IntegerNumber's video.
                    hit_counter = hit_counter + 1 #Registers this click as a successful hit, thus adding to the hit counter variable
                    bonus_counter = bonus_counter + 1 #Runs consecutively with the hit counter to track how many successful hits in a row the player has made
                    reset = True
                    ...
                    click == True
                    if bonus_counter >= 3: #Once the bonus counter reaches 3, the multiplier variable is set to 2 for double points
                        multiplier = 2
                    ...
                    bonus_counter = 0 #Resets the bonus counter back to 0 if the player misses a target
                    multiplier = 1 #Resets the multiplier back to 1 if the player misses a target
...
#Drawing the background color and targets onto the game's screen
        ...
        pygame.display.flip() #Updates the blitted score, timer, accuracy text, and targets to the screen

#Printing the final score and accuracy stats to the console after the game session is complete
    print (f"Final Score: " + str(score)) #Prints the final score to the console after the game window is closed
    print(f"Accuracy Score: " + str(accuracy) + "%") #Prints the final accuracy percentage to the console after the game window is closed
